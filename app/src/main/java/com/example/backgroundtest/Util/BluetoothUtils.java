...
import static com.example.backgroundtest.Util.Constants.CHARACTERISTIC_COMMAND_STRING;
import static com.example.backgroundtest.Util.Constants.CHARACTERISTIC_RESPONSE_STRING;
import static com.example.backgroundtest.Util.Constants.SERVICE_STRING;
...
public class BluetoothUtils {
    ...
    public static List<BluetoothGattCharacteristic> findBLECharacteristics(BluetoothGatt _gatt) {
        ...
        if (service_list != null) {
            Log.d("BlueToothUtils", service_list.toString());
        } else {
            Log.d("BlueToothUtils", "service_list is null");
        }
    ...
    private static BluetoothGattCharacteristic findCharacteristic(BluetoothGatt _gatt, String _uuid_string) {
        ...
        List<BluetoothGattCharacteristic> characteristicList = service.getCharacteristics();
        for (BluetoothGattCharacteristic characteristic : characteristicList) {
            Log.d("characteristic uuid ", characteristic.getUuid().toString());
    ...
    private static BluetoothGattService findGattService(List<BluetoothGattService> _service_list) {
        for (BluetoothGattService service : _service_list) {
            ...
            Log.d("Bluetooth uuid", service_uuid_string);
    ...
    private static boolean isMatchingCharacteristic(BluetoothGattCharacteristic _characteristic) {
        ...
        UUID uuid = _characteristic.getUuid();
        Log.d(" ismathcing uuid : ", uuid.toString());
    ...
    private static boolean matchCharacteristicUUID(String _characteristic_uuid_string) {
        Log.d("uuid", _characteristic_uuid_string);
        Log.d("uuid", CHARACTERISTIC_COMMAND_STRING);
        Log.d("uuid", CHARACTERISTIC_RESPONSE_STRING);
